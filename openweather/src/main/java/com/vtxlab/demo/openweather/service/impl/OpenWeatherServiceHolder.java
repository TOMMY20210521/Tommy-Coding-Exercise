...
public class OpenWeatherServiceHolder implements OpenWeatherService {
  ...
  public CurrentWeatherResponse getCurrentWeather(BigDecimal latitude,
    ...
    if (currentWeatherResponse != null) {
      log.info("########## redis return 1");
      log.info("########## redis return 1 = {}", currentWeatherResponse);
    ...
    try {
      ...
      currentWeatherResponse = restTemplate.getForObject(url, CurrentWeatherResponse.class);
      
      // Set to Redis with 10 minutes expiry
      redisTemplate.opsForValue().set(redisKey, currentWeatherResponse, Duration.ofSeconds(10));
  
//      log.info("redis venturenixlab:java:tutor == {}", redisTemplate.opsForValue().get("venturenixlab:java:tutor"));
      log.info("########## redis return 2");
